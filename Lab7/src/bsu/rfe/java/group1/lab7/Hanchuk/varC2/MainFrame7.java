package bsu.rfe.java.group1.lab7.Hanchuk.varC2;
...
public class MainFrame7 extends JFrame{
	...
	private static MainFrame7 THIS;
	//private static boolean noconnmsg=false;

	MainFrame7(boolean s) throws IOException{
		super("Вход в чат");
		THIS=this;
			...
			textName.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if(!nickIsCorrect(textName.getText())){
						label.setText("Не верный ник");
					}else{
						name=textName.getText();
					}
				}
			});
			final JButton button = new JButton("Войти в чат");
	...
	private void newTab(final String caption, String address){
		...
		panel.add(btn);
		btn.setSize(120,20);
		btn.setLocation(tabbedPane.getWidth()-150,tabbedPane.getHeight()-60);
	...
	private void sendMessage(String nick, String address, int port, JTextArea textAreaMsg, JTextArea textAreaHist) {
		...
		if(message.length()<1 || message.length()>999){
			JOptionPane.showMessageDialog(MainFrame7.this, "Сообщение пустое или слишком длинное",
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					e.printStackTrace();
					JOptionPane.showMessageDialog(MainFrame7.this, "Ошибка в работе сервера", "Ошибка", JOptionPane.ERROR_MESSAGE);
				}
			}
		}).start();
	}
	...
	private void procMSG(String priv, String msg, String addr, String port){
		...
		if(t!=null){
			...
			String s=findUser2(nick);//!если не аторизован то авторизовать
			if(s!=null){
				String[] a=s.split("@");
				newTab(a[0], a[1]);
			}
					...
					while(!Thread.interrupted()){
						...
						if("online".equals(msg)){
							if(findInList(nick,0)==0){
								//String addEl = nick+"@"+ip+":"+port;
								//listModel.addElement(addEl);
	...
	public static void main(String[] args) throws IOException {
		try{
			while(true){
				MainFrame7 frame=new MainFrame7(true);
				...
				JOptionPane.showMessageDialog(frame, "Пользователь "+name+" уже используеься", "Ошибка", JOptionPane.ERROR_MESSAGE);
				frame.setVisible(false);
				frame=null;
				name=null;
				fail=false;
